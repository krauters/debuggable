...
describe('debuggable', () => {
    ...
    test('should log and measure instance method execution', () => {
        ...
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            5, expect.stringContaining('Calling [TestClass.testMethod]')
        )
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            6, expect.stringContaining('Called [TestClass.testMethod]')
        )
    ...
    test('should track method duration for instance methods', () => {
        ...
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            6, 
            expect.stringMatching(/\[Debuggable\] Called \[TestClass\.testMethod\] which returned \[.*\] and took \[\d+\] ms/)
        )             
    })
    ...
    test('should decorate static methods', () => {
        ...
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            5, expect.stringContaining('Calling [TestClass.testStaticMethod]')
        )
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            6, expect.stringContaining('Called [TestClass.testStaticMethod]')
        )
    ...
    test('should log method decoration process', () => {
        ...
        expect(mockLog.debug).toHaveBeenNthCalledWith(
            3, expect.stringContaining(`${prefix}Decorating the following [TestClass] methods with debuggable`)
        )
    })
    ...
    test('should skip decorating if target is not an object or function', () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Target [42] is not an object or function, skipping decoration'))
    })
})
